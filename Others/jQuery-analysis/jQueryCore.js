var jQuery = (function() {
	...
	jQuery.fn = jQuery.prototype = {
		...
		init: function(selector, context, rootjQuery) {
			...
			if (selector.nodeType) {
				this.context = this[0] = selector; //注意这段代码里面设置this.selector和
				this.length = 1;
				return this;
			}
			...
			if (typeof selector === "string") {
				...
				if (match && (match[1] || !context)) { //match[1]存在说明有html，如果不存在，则说明是id，就不需要context。
					...
					// HANDLE: $(html) -> $(array)
					if (match[1]) { //处理html
						context = context instanceof jQuery ? context[0] : context; //jQuery类型的变量将DOM元素存在jQuery[0]中
						...
						return jQuery.merge(this, selector); //返回this（jQuery对象）。
	...
	jQuery.extend = jQuery.fn.extend = function() {
		...
		if (typeof target !== "object" && !jQuery.isFunction(target)) { //target不是对象或者函数，即不能表示为对象的。
			target = {};
		}
		...
		for (; i < length; i++) { //target之后的所有参数都是复制的对象
			...
			if ((options = arguments[i]) != null) {
				...
				for (name in options) {
					...
					// Recurse if we're merging plain objects or arrays //如果是普通的对象或者数组，就递归的深度复制。
	...
	jQuery.extend({
		...
		type: function(obj) {
			return obj == null ? String(obj) : class2type[toString.call(obj)] || "object"; //String()方法将其变为字符串，类似于toString().class2type中保存着各个对象toString的字符串。
		},
