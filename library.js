...
function hasPlugin(name){
	...
	if(window.ActiveXObject){
		...
		var pulgin;
		for(var i = 0;(plugin = plugins[i]); i++){
...
function client(){
	var engine = {
		ie:0,gecko:0,webkit:0,khtml:0,opera:0,ver:null
	}
	var browser = {
		ie:0,firefox:0,konq:0,opera:0,chrome:0,safari:0,ver:null
	}
	var system = {
		win:false,mac:false,x11:false,iphone:false,ipod:false,nokiaN:false,winMobile:false,macMobile:false,wii:false,ps:false
	}
	...
	if(window.opera){
		...
		if(/Chrome\/(\S+)/i.test(ua)){
			...
			browser.ver = RegExp['$1'];
			browser.safari = parseFloat(browse.ver);
		}else{
			var safariVersion = 1;
